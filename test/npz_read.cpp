...
void _test_large(int &result, const std::string &filename, bool compressed) {
  ...
  npy::npzfilereader stream(test::asset_path(filename));
  auto actual_int = stream.read<npy::tensor<std::int32_t>>("test_int");
  auto actual_float = stream.read<npy::tensor<float>>("test_float");

  std::string suffix = compressed ? "_compressed" : "";
  test::assert_equal(expected_int, actual_int, result,
                     "npz_read_large_int" + suffix);
  test::assert_equal(expected_float, actual_float, result,
                     "npz_read_large_float" + suffix);
}

void _test_memory(int &result, const std::string &filename) {
  std::ifstream input(test::asset_path(filename),
