...
void _test(int &result, const std::string &filename, bool compressed)
{
    ...
    test::assert_equal(keys[0], std::string("color.npy"), result, "npz_keys_incorrect");
    test::assert_equal(keys[1], std::string("depth.npy"), result, "npz_keys_incorrect");
    test::assert_equal(keys[2], std::string("unicode.npy"), result, "npz_keys_incorrect");
