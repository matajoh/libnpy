 ...
 * endianness) \param fortran_order whether the data is encoded in FORTRAN (i.e.
 * column major) order \param shape a sequence of values indicating the shape of
 * each dimension of the tensor \sa npy::to_dtype
 */
...
void copy_to(const T *data_ptr, std::size_t num_elements,
             std::basic_ostream<CHAR> &output, npy::endian_t endianness) {
  if (endianness == npy::endian_t::NATIVE || endianness == native_endian()) {
    output.write(reinterpret_cast<const CHAR *>(data_ptr),
...
template <typename T, template <typename> class TENSOR, typename CHAR,
          std::enable_if_t<std::is_same<std::wstring, T>::value, int> = 42>
void save(std::basic_ostream<CHAR> &output, const TENSOR<std::wstring> &tensor,
          endian_t endianness = npy::endian_t::NATIVE) {
...
void save(const std::string &path, const TENSOR<T> &tensor,
          endian_t endianness = npy::endian_t::NATIVE) {
...
template <typename T, typename CHAR>
void copy_to(std::basic_istream<CHAR> &input, T *data_ptr,
             std::size_t num_elements, npy::endian_t endianness) {
  if (endianness == npy::endian_t::NATIVE || endianness == native_endian()) {
...
/** Loads a tensor in NPY format from the provided stream. The type of the
 * tensor must match the data to be read. \tparam T the data type \tparam TENSOR
 * the tensor type \param input the input stream \return an object of type
 * TENSOR<T> read from the stream \sa npy::tensor
 */
template <typename T, template <typename> class TENSOR, typename CHAR,
          std::enable_if_t<!std::is_same<std::wstring, T>::value, int> = 42>
...
/** Loads a unicode string tensor in NPY format from the provided stream. The
 * type of the tensor must match the data to be read. \tparam T the data type
 *  \tparam TENSOR the tensor type
 *  \param input the input stream
...
TENSOR<T> load(std::basic_istream<CHAR> &input) {
  ...
  for (auto &element : tensor) {
    auto char_it = word_start;
    for (std::size_t i = 0; i < info.max_element_length && *char_it > 0;
         ++i, ++char_it) {
...
/** Loads a tensor in NPY format from the specified location on the disk. The
 * type of the tensor must match the data to be read. \tparam T the data type
 *  \tparam TENSOR the tensor type
