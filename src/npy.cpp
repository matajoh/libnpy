  ...
  while (input.peek() != '}') {
    ...
    if (key == "descr") {
      ...
      if (dtype_code[1] == 'U') {
        ...
        endianness =
            dtype_code[0] == '>' ? npy::endian_t::BIG : npy::endian_t::LITTLE;
...
header_info::header_info(data_type_t dtype, npy::endian_t endianness,
                         bool fortran_order, const std::vector<size_t> &shape) {
  this->dtype = dtype;
