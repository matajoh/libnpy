...
namespace npy {
     ...
     *  \sa npy::to_dtype
     */
    void write_npy_header(std::ostream& output, const std::string& dtype, bool fortran_order, const std::vector<size_t>& shape);
    ...
    void save(std::ostream& output, const TENSOR<T>& tensor, endian endianness=npy::endian::NATIVE)
    {
        auto dtype = to_dtype(tensor.dtype(), endianness);
        write_npy_header(output, dtype, tensor.fortran_order(), tensor.shape());
        
        if(endianness == npy::endian::NATIVE || endianness == native_endian() || dtype[0] == '|'){
    ...
    template <typename T,
              template<typename> class TENSOR>
    void save(const std::string& path, const TENSOR<T>& tensor, endian endianness=npy::endian::NATIVE)
    {
